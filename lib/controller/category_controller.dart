...
class CategoryController extends GetxController implements GetxService {
  ...
  dynamic get subCategoryIndex => _subCategoryIndex;
  ...
  Future<void> getCategoryList(bool reload) async {
    if(_categoryList == null || reload) {
      ...
      if (response.statusCode == 200) {
        ...
        if(kDebugMode){
          // print("category list is ${_categoryList.length}");
        }
        });
      } else {
  ...
  void getSubCategoryList(String categoryID) async {
    ...
    if (response.statusCode == 200) {
      ...
      if(kDebugMode){
        print("this is sub category list ${response.body}");
      }
      _subCategoryList= [];
  ...
  void setSubCategoryIndex(dynamic index, String categoryID) {
  ...
  void getCategoryProductList(dynamic categoryID, int offset, String type, bool notify) async {
    _offset = offset;
    if(offset == 1) {
      if(_type == type) {
        _isSearching = false;
    ...
    if (response.statusCode == 200) {
      if(kDebugMode){
        print("this is product category list ${response.body}");
      }
