...
import 'package:Apka_kitchen/data/model/response/distance_model.dart';
import 'package:Apka_kitchen/data/model/response/order_details_model.dart';
import 'package:Apka_kitchen/data/model/response/order_model.dart';
import 'package:Apka_kitchen/data/model/response/response_model.dart';
import 'package:Apka_kitchen/data/model/response/restaurant_model.dart';
import 'package:Apka_kitchen/data/model/response/timeslote_model.dart';
  ...
  Future<void> getHistoryOrders(int offset) async {
    ...
    if (!_historyOffsetList.contains(offset)) {
      ...
      if (response.statusCode == 200) {
        ...
        if(kDebugMode){
          print("history list is ${response.body}");
        }
        update();
      } else {
        ApiChecker.checkApi(response);
    ...
    if(_minutes > AppConstants.SLOT_IN_MINUTE) {
      ...
      for(;;) {
        if(_time.isBefore(_closeTime)) {
          ...
          DateTime _end = _start.add(Duration(minutes: AppConstants.SLOT_IN_MINUTE));
          if(_end.isAfter(_closeTime)) {
            _end = _closeTime;
          }
          _timeSlots.add(TimeSlotModel(startTime: _time, endTime: _time.add(Duration(minutes: AppConstants.SLOT_IN_MINUTE))));
          _allTimeSlots.add(TimeSlotModel(startTime: _time, endTime: _time.add(Duration(minutes: AppConstants.SLOT_IN_MINUTE))));
